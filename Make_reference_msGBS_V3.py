...
def join_fastq(r1,r2,outfile,args):
    ...
    cmd = ["paste <(seqtk seq %s | cut -c1-%s) " % (r1, max_len_R1) +
           "<(seqtk seq -r %s |cut -c1-%s|seqtk seq -r -)|cut -f1-5" % (r2, max_len_R2)+
           "|sed '/^@/!s/\t/NNNNNNNN/g'| sed s/+NNNNNNNN+/+/g|pigz -p %s -c > %s" % (args.threads, outfile)]
