...
import Levenshtein #Had met installeren wat problemen maar : SUDO /usr/bin/python -m pip install python-levenshtein lukte wel
#Thomas zegt : /usr/bin/env python is nog beter
import re
import sys
import os
import shutil
...
def parse_options():
    ...
    parser.add_option("-o", "--output_dir", metavar="outputdir", action="store",
    ...
    parser.add_option("-s", "--split", action="store_true",
                      ...
                      help="Create multiple output files") #werkt niet
...
def parse_bc(barcodes, fc, ln):
    ...
    for line in file_in.readlines():
        if line.startswith("#") or line.startswith("Flowcell"):
            ...
            bc_instance = Barcode() #Dit was even zoeken
            for n, item in enumerate(line.rstrip('\n').split('\t')):
            ...
            if bc_instance.ENZ_R1 != None:
                ...
                bc_instance.ENZ_R1 = get_enz('PstI') #Default
            if bc_instance.ENZ_R2 != None:
                bc_instance.ENZ_R2 = get_enz(bc_instance.ENZ_R2)
            else:
                bc_instance.ENZ_R2 = get_enz('PstI') #Default
...
if __name__ == "__main__":
    print "This version outputs for NGmerge make reference script NOT for PEAR make reference scripts !!!"
