...
#NOTE: script marks duplicate and low alignment reads; these markings are used in msGBS_STATS.py
...
def parse_args():
    ...
    parser.add_argument('-T', '--samsortsuffix',
                        help='suffix for samtools sort')
...
def remove_PCR_duplicates(bam_in, bam_out, ref, stat):
    ...
    for cluster in clusters:
        #TODO: detect enzyme from barcode file to enable other combinations.>changed Csp6I to PacI
        enzymes = ["PacI","NsiI"]
            ...
            for read in reads:
                ...
                if tag not in read_out[sample]:
                    ...
                    try:
                        ...
                        read_out[sample][tag][read.qname] = AS
                if tag_dict['AS'] < 0.8 * cluster_seq_len:
    ...
    if stat:
        f1=open(stat, 'w+')
        print('Sample:\tReads:\tDuplicates:\tDuplicate rate:', file=f1)
        ...
        print('Sample:\tReads:\tQC-fail:\tQC-fail rate:', file=f1)
        for key , subdict in sorted(read_count.items()):
            count = subdict['count']
...
def main():
    ...
    if not args.input and args.input_dir:
        ...
        remove_PCR_duplicates(args.input, args.output, args.reference, args.stat)
